...
# for i in range(1000) :
    ...
    packet_number = int(id_field[4:16],2)
    print(packet_number)

    # 1. skip invalid package
    if packet_number & 0x0f == 0x0f:
        continue    
    
    # 2. read full frame (60 package)
    # if packet_number < int('0xf00',16) :
        # print(packet_number)
    odd_index_byte = np.left_shift(data[4::2],8)
    even_index_byte = np.array(data[5::2])
    try:
        if not frame[packet_number].any() :
...
# Normalize array value to 16 bit data [0-65535] , you can choose 8-bit to
normed_frame = cv2.normalize(frame,None,0,65535,cv2.NORM_MINMAX ) 
# normed_frame = cv2.normalize(frame,None,0,255,cv2.NORM_MINMAX, cv2.CV_8U ) 

# bgr_frame = cv2.cvtColor(normed_frame,cv2.COLOR_GRAY2BGR)
# color_frame = cv2.LUT((bgr_frame).astype(np.uint8),colorMap)

# cv2.imwrite("grey.png",normed_frame)
# cv2.imwrite("color.png",cv2.resize(color_frame,(320,240),cv2.INTER_AREA))
# cv2.imwrite("grey.png",cv2.resize(bgr_frame,(320,240),cv2.INTER_AREA))
# end = time.time()
# print(end - start)

cv2.imshow('flir',normed_frame)
cv2.waitKey(0)